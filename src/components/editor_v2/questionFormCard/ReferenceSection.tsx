...
import { ChevronDown, ChevronRight, CheckCircle, Globe, Video, ImageIcon, Mic, FileDigit, FileText, LockIcon, Shield } from 'lucide-react';
...
interface ReferenceSectionProps {
  ...
  requireRef: boolean;
  linkedRefs: any[];
  isRefExpanded: boolean;
  availableRefs: any[];
  draftSelectedRefIds: string[];
  draftPageByRefId: Record<string, string>;
  draftPageErrors: Record<string, string>;
  hasError: boolean;
  handleToggleReferenceEditor: () => void;
  handleToggleDraftReference: (id: string) => void;
  handleDraftPageChange: (id: string, val: string) => void;
  handleUpdateReferences: () => void;
}
...
export const ReferenceSection: React.FC<ReferenceSectionProps> = ({
  ...
  draftPageErrors,
  hasError,
  handleToggleReferenceEditor,
  handleToggleDraftReference,
  handleDraftPageChange,
  handleUpdateReferences,
}) => {
  if (isDefaultL1 || !requireRef) return null;
