...
import { CheckCircle, GripVertical, ListChecks, Trash2 } from "lucide-react";
...
export const SubQuestionListEditor: React.FC<SubQuestionListEditorProps> = ({
  sqClr,
  useSubQuestions,
  setUseSubQuestions,
  setActiveSubQCodes,
      ...
      {useSubQuestions && (
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <div className="min-w-[140px] max-w-[280px] w-fit">
              ...
              {!sectionOccupationBranches ? (
                ...
                <Tooltip content="แสดงค่าที่เลือกมาจากข้อย่อย 2xx.2" position="top-start" className="w-full">
                  <select
                    value={selMainBranch}
                    disabled
              ...
              <div className="min-w-[140px] max-w-[280px] w-fit">
                ...
                {!sectionOccupationBranches ? (
                  ...
                  >
                    <select
                      value={selSubBranch}
                      onChange={(e) => setSelSubBranch(e.target.value)}
                      className={`w-full px-2 py-1.5 text-xs border ${sqClr.selectBd} rounded bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 outline-none ${sectionSelectedBranch ? "cursor-not-allowed opacity-80" : ""}`}
                      disabled={!!sectionSelectedBranch}
                    >
                      <option value="">-- เลือก --</option>
            ...
            {autoCodePrefix && (
              <div className="shrink-0">
                ...
                <div className={`px-2 py-1.5 text-xs font-mono font-bold ${sqClr.code} rounded`}>{autoCodePrefix}</div>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 gap-0.5">
              {filteredItems.map((item, localIdx) => {
                ...
                return (
                  <div
                    key={item.code}
                    className={`flex items-center gap-2 px-2 py-1 bg-white dark:bg-slate-900/60 border ${sqClr.itemBd} rounded-md group/sq-item`}
                  >
                    <GripVertical className="w-3 h-3 text-slate-300 dark:text-slate-600 shrink-0" />
                    <span className={`text-xs font-bold ${sqClr.itemText} min-w-[1.5ch]`}>
                      {toThaiAlphabet(localIdx + 1)}.
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded shrink-0">
                      {item.code}
                    </span>
                    <span className="flex-1 text-xs text-slate-700 dark:text-slate-200 truncate">{item.text}</span>
                    {showForcedBadge && (
                      <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded-full shrink-0">
                        บังคับ ✓
                      </span>
                    )}
                    {showActionButtons && (
                      <div className="flex items-center gap-0.5 opacity-0 group-hover/sq-item:opacity-100 transition-opacity">
                        {is300 && (
                          <Tooltip content={item.alwaysChecked ? "ยกเลิกบังคับ" : "บังคับเลือกเสมอ"}>
                            <button
                              onClick={async () => {
                                if (dbSq) {
                                  const newAc = !item.alwaysChecked;
                                  await invoke("update_occupation_sub_question", {
                                    id: dbSq.id,
                                    text: dbSq.text,
                                    alwaysChecked: newAc,
                                  });
                                  setDbSubQuestions((prev) =>
                                    prev.map((s) => (s.id === dbSq.id ? { ...s, always_checked: newAc } : s)),
                                  );
                                  if (newAc && useSubQuestions) {
                                    setActiveSubQCodes((prev) => Array.from(new Set([...prev, item.code])));
                                  }
                                } else {
                                  const gi = subQuestionList.findIndex((sq) => sq.code === item.code);
                                  const u = [...subQuestionList];
                                  const ugi = u[gi];
                                  if (ugi) {
                                    u[gi] = { ...ugi, alwaysChecked: !ugi.alwaysChecked };
                                    setSubQuestionList(u);
                                  }
                                  if (!item.alwaysChecked && useSubQuestions) {
                                    setActiveSubQCodes((prev) => Array.from(new Set([...prev, item.code])));
                                  }
                                }
                              }}
                        ...
                        <Tooltip content="ลบ">
                          <button
                            onClick={async () => {
                              if (dbSq) {
                                await invoke("delete_occupation_sub_question", { id: dbSq.id });
                                setDbSubQuestions((prev) => prev.filter((s) => s.id !== dbSq.id));
                              } else {
                                setSubQuestionList((prev) => prev.filter((sq) => sq.code !== item.code));
                              }
                            }}
          ...
          {filteredItems.length > 0 && autoCodePrefix && (() => {
            ...
            const activeInBranch = Array.from(
              new Set([...activeSubQCodes.filter((c) => branchCodes.includes(c)), ...alwaysCodes]),
            );
            ...
            return (
              <div className={`pt-2 border-t ${sqClr.border}`}>
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  <span className={`text-xs font-bold ${sqClr.textBold} uppercase tracking-wider flex-1`}>
                    เลือกข้อย่อยที่ใช้งาน
                  </span>
                  <span className={`text-[10px] ${sqClr.count}`}>
                    {activeInBranch.length}/{filteredItems.length}
                  </span>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setActiveSubQCodes((prev) => [
                          ...prev.filter((c) => !branchCodes.includes(c)),
                          ...branchCodes,
                        ])
                      }
                      className={`px-2 py-0.5 text-[10px] font-bold rounded border transition-colors ${allActive ? sqClr.activeAll : sqClr.addBtn}`}
                    >
                      เลือกทั้งหมด
                    </button>
                    <button
                      ...
                      className="px-2 py-0.5 text-[10px] font-bold rounded border border-slate-300 dark:border-slate-600 text-slate-500 hover:bg-slate-100"
                    >
                      ยกเลิกทั้งหมด
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-0.5">
                  {filteredItems.map((sq, idx) => {
                    ...
                    return (
                      <label
                        key={sq.code}
                        className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer select-none text-xs ${isActive ? sqClr.activeBg + " text-slate-700 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"} ${isForced ? "opacity-80" : ""}`}
                      >
                        <input
                          type="checkbox"
                          checked={isActive}
                          ...
                          onChange={() => {
                            if (isForced) return;
                            setActiveSubQCodes((prev) =>
                              isActive ? prev.filter((c) => c !== sq.code) : [...prev, sq.code],
                            );
                          }}
                          className={`w-3 h-3 rounded ${sqClr.check}`}
                        />
                        <span className={`font-bold ${sqClr.textBold} min-w-[1.5ch]`}>
    ...
    <div className={`rounded-lg border ${sqClr.bindWrap} p-3`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-bold ${sqClr.textBold} uppercase tracking-wider`}>
            เลือกคำถามย่อย (Select Sub-Questions)
          </span>
          <span className={`text-[10px] ${sqClr.count}`}>
        ...
        <div className="flex items-center gap-1">
          <Button
            variant={
              selectedSubQCodes.length > 0 &&
              selectedSubQCodes.length < parentSubQuestionList.filter((sq) => !(is300 && sq.alwaysChecked === true)).length
                ? "primary"
                : "outline"
            }
            size="small"
            onClick={() => {
              const allCodes = parentSubQuestionList
                .filter((sq) => !(is300 && sq.alwaysChecked === true))
                .map((sq) => sq.code);
              setSelectedSubQCodes(allCodes);
            }}
            disabled={
              selectedSubQCodes.length ===
              parentSubQuestionList.filter((sq) => !(is300 && sq.alwaysChecked === true)).length
            }
      ...
      <div className="grid grid-cols-1 gap-1">
        {parentSubQuestionList.map((sq, idx) => {
          ...
          return (
            <label
              key={sq.code}
              className={`flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer select-none text-xs ${isChecked ? sqClr.activeBg + " text-slate-700 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"} ${isForced ? "opacity-70" : ""}`}
            >
              <input
                type="checkbox"
                checked={isChecked}
                ...
                onChange={() => {
                  ...
                  setSelectedSubQCodes((prev) =>
                    isChecked ? prev.filter((c) => c !== sq.code) : [...prev, sq.code],
                  );
                }}
                className={`w-3 h-3 rounded ${sqClr.check}`}
              />
              ...
              {!isForced && (() => {
                ...
                if (count === 0) {
                  return (
                    <span className="ml-auto text-[10px] px-2 py-[1px] rounded-full bg-orange-100 dark:bg-orange-900/10 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-800/50 font-bold whitespace-nowrap">
                      Unused (0/{total})
                    </span>
                  );
                }
                return (
                  <span className="ml-auto text-[10px] px-2 py-[1px] rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800 font-bold whitespace-nowrap">
                    Used: {count}/{total}
                  </span>
                );
              })()}
