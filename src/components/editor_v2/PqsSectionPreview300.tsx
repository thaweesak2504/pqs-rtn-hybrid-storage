...
const PreviewQuestionNode300: React.FC<PreviewQuestionNode300Props> = ({
  ...
  const contentOffsetClass = (level === 0 || level === 1) ? 'ml-[9ch]' : 'ml-[2ch]';

  return (
    <div className="flex flex-col">
      {/* Question Row */}
      <div className={`flex items-baseline ${isL3 ? 'text-slate-700 dark:text-slate-300' : ''}`}>
        <span className={`${(level === 0 || level === 1) ? 'min-w-[9ch]' : 'min-w-[2ch] mr-1'} ${question.is_header ? 'font-bold' : 'font-normal'} ${isL3 ? 'text-orange-700 dark:text-orange-400' : ''} shrink-0`}>
          {displayNumber}
        </span>
