...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  const arrowClasses = {
    ...
    'top-start': 'top-full left-4 border-t-slate-200 dark:border-t-slate-700 border-l-transparent border-r-transparent border-b-transparent',
    'top-end': 'top-full right-4 border-t-slate-200 dark:border-t-slate-700 border-l-transparent border-r-transparent border-b-transparent',
    ...
    'bottom-start': 'bottom-full left-4 border-b-slate-200 dark:border-b-slate-700 border-l-transparent border-r-transparent border-t-transparent',
    'bottom-end': 'bottom-full right-4 border-b-slate-200 dark:border-b-slate-700 border-l-transparent border-r-transparent border-t-transparent',
    ...
    <div className={`relative group inline-block ${className}`}>
      ...
      <div className={`absolute z-[9999] whitespace-nowrap px-3 py-2 text-xs font-normal text-slate-500 dark:text-slate-300 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border border-slate-200 dark:border-slate-700 rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none ${positionClasses[position]}`}>
        ...
        <div className={`absolute border-[5px] w-0 h-0
          ${position.startsWith('top') ? `top-full -mt-[2px] border-t-white dark:border-t-slate-900 border-l-transparent border-r-transparent border-b-transparent ${position === 'top' ? 'left-1/2 -translate-x-1/2' : position === 'top-start' ? 'left-4' : 'right-4'}` : ''}
          ${position.startsWith('bottom') ? `bottom-full -mb-[2px] border-b-white dark:border-b-slate-900 border-l-transparent border-r-transparent border-t-transparent ${position === 'bottom' ? 'left-1/2 -translate-x-1/2' : position === 'bottom-start' ? 'left-4' : 'right-4'}` : ''}
