...
> **สร้างเมื่อ:** 2026-04-20 | **อัปเดตล่าสุด:** 2026-04-29
...
| **Phase 2C** — Connection Pooling                  | ✅ **DONE**    | `e06a292`                            | r2d2 pool + per-connection PRAGMA customizer; `get_content_connection` → pooled `DbConn`; รองรับ WAL/FK/busy_timeout อัตโนมัติ             |
...
| **Phase 5** — Large File Refactoring               | 🔴 **REVERTED** | `6097db0` → `7967e0f`              | ลอง refactor ครั้งแรก แต่พบ regression หลายจุดใน 200 Section → revert กลับ ต้องวางแผนใหม่แบบ incremental                                     |
| **UX/UI** — Empty State Improvement                | ✅ **DONE**    | `cb166c7`                            | ปรับ UX/UI ของ Empty State (เอกสารอ้างอิง + คำถาม) ใน Section 100 ให้เป็น Command Button style — ยังคงอยู่หลัง revert                       |
...
1. **Phase 5 — Large File Refactoring (วางแผนใหม่)** ต้องทำแบบ incremental: ทีละไฟล์ ทีละส่วน พร้อม Manual Test ทุกขั้นตอน
2. ⚠️ **บทเรียนสำคัญ:** การ Refactor ทั้ง QuestionFormCard + Rust commands + Tests พร้อมกัน commit เดียวทำให้ debug regression ได้ยากมาก
3. เคลียร์ React Hooks Warnings (37 warnings) ของ ESLint ในช่วงเวลาว่าง
...
6. 🟢 **Type safety** — `any` หายไป 100% (Phase 7), `console.log` เปลี่ยนเป็น `logger` (Phase 4)
7. 🟡 **Dead code** — ลบไปแล้วส่วนใหญ่ แต่ยังอาจมีหลงเหลือบ้างประปราย → Phase 4
...
| `console.log`    | 🟢    | 0 ครั้งใน production (เปลี่ยนไปใช้ `utils/logger.ts` หมดแล้ว)                                                                    |
| `any` types      | 🟢    | 0 ครั้งใน production code (แก้ไข 93 จุดเสร็จสิ้น 100%)                                                                             |
...
### ✅ Phase 4: Dead Code Cleanup (1 week) — DONE (2026-04-21, commit `953d64f`)
...
- [x] ลบ disabled database logging commands ใน `main.rs` สำเร็จ (~22 lines)
- [x] `cargo clippy --fix` clean — auto-fixed `auth.rs` + `universal_sqlite_backup.rs`
- [x] แทนที่ `console.log/warn/error` 305 ครั้ง ด้วย `logger.ts` ใน 70 ไฟล์
- [x] รวม `avatarService.ts` + `avatarDatabaseService.ts` → `hybridAvatarService.ts`
- [x] ลบ `useAvatar.ts`, `useAvatarDatabase.ts` hooks ที่ซ้ำซ้อน
- [x] ดึง `*ContextObject.ts` 5 ไฟล์เข้า Context.tsx หลัก (Auth, Breadcrumb, DarkMode, SlideBar, UserProfile)
- [x] ทุก 175 tests ผ่าน + TypeScript compile clean

---


### 🔴 Phase 5: Large File Refactoring (2-3 weeks) — REVERTED, NEEDS REPLANNING

> [!CAUTION]
> **Phase 5 ถูก Revert (2026-04-29):** Commit `6097db0` ทำ refactoring ครั้งใหญ่ (QuestionFormCard.tsx → modular + main.rs → commands/* + tests.rs → modules) ใน commit เดียว
> พบ regression หลายจุดใน 200 Section หลัง Manual Testing:
> 1. ชื่อคำถาม 2xx.1-2xx.8 ที่ควร fixed กลายเป็นแก้ไขได้ (ขาด `disabled` prop)
> 2. Description หายไปสำหรับ 2xx.2 และ 2xx.4
> 3. SubQuestionList ของ 2xx.2 และ 2xx.4 ใช้ข้อมูลเดียวกัน (ใช้ `isProtectedBranch` แทน `sectionOccupationBranches`)
>
> **Revert commit:** `7967e0f` — กลับไปเป็นโค้ดก่อน refactoring โดยเก็บ UX/UI improvements ไว้

**เป้าหมาย:** ไม่มีไฟล์เกิน 1,500 lines

**แนวทางใหม่ (Incremental):**
- ❌ ห้ามทำหลายไฟล์พร้อมกันใน commit เดียว
- ✅ ทำทีละไฟล์ ทีละส่วนย่อย
- ✅ Manual Test ทุก section (100, 200, 300) หลังทำแต่ละ commit
- ✅ Commit + Push ทันทีที่ผ่าน test เพื่อ rollback ได้ง่าย

| #   | Target File                                   | Plan                                                                              | สถานะ        |
| --- | --------------------------------------------- | --------------------------------------------------------------------------------- | ------------ |
| 5.1 | `QuestionFormCard.tsx` (138 KB, ~3,200 lines) | แยกเป็น: Form logic, Validation, Sub-question UI, Reference UI, Image upload      | 🔴 Reverted  |
| 5.2 | `main.rs` (49 KB, 150 commands)               | แยกเป็น `commands/{users,documents,sections,questions,scoring,backup,avatars}.rs` | 🔴 Reverted  |
| 5.3 | `tests.rs` (82 KB)                            | แยกตาม module: `tests/{sections,questions,scoring,branches,cleanup}.rs`           | 🔴 Reverted  |
| 5.4 | `schema.rs` (47 KB)                           | หลัง Phase 2 จะเล็กลงมาก (move to migrations/)                                    | ⏳ pending   |
| 5.5 | `CareerBranchManagerModal.tsx` (49 KB)        | แยก sub-components                                                                | ⏳ pending   |
| 5.6 | `PqsReferenceSection.tsx` (45 KB)             | แยก hook + presentation                                                           | ⏳ pending   |
| 5.7 | `AddReferenceModal.tsx` (34 KB)               | แยก form + list                                                                   | ⏳ pending   |

**Deliverables:**

- [ ] ทุกไฟล์ < 1,500 lines
- [ ] Tests ทั้งหมดยังผ่าน (no regression)
- [ ] Manual Test ทุก section (100, 200, 300) ผ่านหลังแต่ละ commit

---

### ✅ Phase 6: Database Consolidation Cleanup — DONE (verified 2026-04-21, commit `558d3ec`+)

**สถานะ:** ✅ **ทุก cleanup task เสร็จสิ้นแล้ว**

**สิ่งที่เสร็จแล้ว:**

- ✅ `users`, `high_ranking_officers` ย้ายเข้า `content.db` แล้ว
- ✅ ทุก `get_database_path()` ชี้ไปที่ `content.db`
- ✅ `initialize_content_database()` สร้างทุกตารางรวม
- ✅ Backup restore รองรับทั้ง old format (`database.db`) และ new (`content.db`)

**ผลการตรวจสอบ cleanup tasks:**

| #   | Task                                                                                            | สถานะ   |
| --- | ----------------------------------------------------------------------------------------------- | ------- |
| 6.1 | รวม `get_database_path()` ซ้ำซ้อน → ใช้ `connection::get_content_database_path()` เดียว         | ✅ Done  |
| 6.2 | Rename `database.rs` → `content_database/` module                                              | ✅ Done  |
| 6.3 | ลบ historical consolidation comments                                                             | ✅ Done  |
| 6.4 | Cleanup legacy `database.db` files บน disk ของ user เก่า                                       | ✅ Done (`migration_helper.rs` + `main.rs`) |
| 6.5 | Fallback `database.db` ใน `hybrid_backup.rs` — เพิ่ม TODO comment, retain ไว้จนถึง v0.2.x     | ✅ Done (deferred intentionally) |
| 6.6 | อัปเดต `database_consolidation_plan.md` → archived ไปที่ `docs/archive/`                        | ✅ Done  |

**Deliverables:**

- [x] 1 path helper (source of truth = `content_database::connection::get_content_database_path`)
- [x] `database.rs` rename/relocate → `content_database/` module
- [x] Historical comments ล้างออก
- [x] Plan doc archived (`docs/archive/database_consolidation_plan.md`)
...
| 2C. Connection Pooling                | 1-2 weeks | 🟠 High     | ✅ **DONE**    | Phase 2A             |
| 3. Backup/Export Consolidation        | 1-2 weeks | 🟠 High     | ✅ **DONE**    | -                    |
| 4. Dead Code Cleanup                  | 1 week    | 🟡 Medium   | ✅ **DONE**    | Phase 3 (บาง module) |
| 7. Type Safety & DX (Part 1)          | 1 week    | 🟡 Medium   | ✅ **DONE**    | -                    |
| 5. Large File Refactoring             | 2-3 weeks | 🟡 Medium   | 🔴 **REVERTED** | Phase 2A (schema)  |
...
**คืบหน้า:** 6 phases เสร็จแล้ว (1, 2, 2C, 3, 4, 6, 7-Part1) + 1 phase reverted (5) + 1 phase deferred (2B). Phase 5 ต้องวางแผนใหม่แบบ incremental. **Remaining estimate:** ประมาณ 6-10 weeks sequential / **4-7 weeks** ถ้าทำ parallel บางส่วน
...
✅ Week 4:     Phase 2C (Conn Pooling)   DONE (commit e06a292)
✅ Week 5:     Phase 3 (Backup merge)     DONE (commit 326a59d)
✅ Week 5:     Phase 4 (Dead code)        DONE (commit 953d64f)
✅ Week 6:     Phase 7 (Type Safety)      DONE (กำจัด any 93 ตัว + tsconfig strict)
🔴 Week 6-7:   Phase 5 (Large files)     REVERTED (6097db0 → 7967e0f) — ต้องวางแผนใหม่แบบ incremental
✅ Week 7:     UX/UI Empty State          DONE (commit cb166c7) — เก็บไว้หลัง revert
⏳ Week 8+:    Phase 5 (Redo)            🟠 ทำใหม่แบบทีละชิ้น + Manual Test ทุกขั้นตอน
   Week 9-10:  Phase 2B (CREATE TABLE)    🟡 optional, มีค่าเมื่อ schema นิ่งแล้ว
   Week 14-15: Phase 7.2 (Shared Types)   🟢
...
### Current (ปัจจุบัน หลัง Phase 7 Part 1 + Phase 5 Revert)

- Rust tests: **98** (+22)
- FE tests: **175** (+17)
- `any` types: ✅ **0** (Type safety 100% strict)
- `console.log`: ✅ **0** (ใช้ logger หมดแล้ว)
...
- UX/UI Empty State: ✅ Command Button style (Section 100, เอกสารอ้างอิง + คำถาม)
- Phase 5 Refactoring: 🔴 Reverted — ต้องวางแผนใหม่แบบ incremental

### Target (เป้าหมายถัดไป Phase 5)

- Largest file: < 50 KB (โดยเฉพาะ `QuestionFormCard.tsx` และ `main.rs`)
- Backup modules: 1
- Connection pool: มี
