...
1. **Phase 5G — Export/Import Testing** ทดสอบว่า Trainee Attachments ถูก bundle เข้า `.pqs.zip` ได้ถูกต้อง และ Import กลับมาใช้งานได้
2. **Phase 5.5-5.7** — แยก sub-components ของ `CareerBranchManagerModal`, `PqsReferenceSection`, `AddReferenceModal` (optional, ความเสี่ยงต่ำ)
3. **Phase 7.3-7.4** — Shared Types (Rust ↔ TS) + JSDoc
4. **Phase 8** — Performance & Optimization
5. เคลียร์ React Hooks Warnings (37 warnings) ของ ESLint ในช่วงเวลาว่าง
...
### ✅ Phase 5: Large File Refactoring — DONE (2026-05-01 ~ 2026-05-02)

> [!NOTE]
> **สถานะ:** Phase 5A-5F เสร็จสิ้นทั้งหมด + Phase 5G (Trainee Attachments Improvements) เสร็จ 2026-05-13
...
| 5.5 | `CareerBranchManagerModal.tsx`                | แยก sub-components                                                                | ⏳ pending (optional) |
| 5.6 | `PqsReferenceSection.tsx`                     | แยก hook + presentation                                                           | ⏳ pending (optional) |
| 5.7 | `AddReferenceModal.tsx`                       | แยก form + list                                                                   | ⏳ pending (optional) |
| **5G** | **Trainee Attachments + Qualifier UX**    | AttachmentPanel, SHA-256, Prefix, Qualifier Undo (OralAssessmentBox + TraineeAnswerBox) | ✅ Done |
...
| 5. Large File Refactoring (5A-5F)     | 2-3 weeks | 🟡 Medium   | ✅ **DONE**    | Phase 2A (schema)    |
...
**คืบหน้า:** 8 phases เสร็จแล้ว (1, 2, 2C, 3, 4, 5, 5G, 6, 7-Part1) + 1 phase deferred (2B). **Remaining:** Phase 5.5-5.7 (optional), Phase 7 Part 2, Phase 8-10
...
### Current (ปัจจุบัน หลัง Phase 5G — 2026-05-13)
...
- Phase 5 Refactoring: ✅ Done (5A-5F: `main.rs` 1544→267 lines, QFC 2608→~1550 lines)
- Phase 5G Attachments: ✅ Done (AttachmentPanel, SHA-256, Prefix, Qualifier UX Undo)
- Qualifier UX: ✅ ปุ่มยกเลิกผ่าน + ล้าง Comment ทั้ง TraineeAnswerBox และ OralAssessmentBox

### Target (เป้าหมายถัดไป)

- Export/Import ทดสอบ Trainee Attachments ใน `.pqs.zip`
- Shared Types Rust ↔ TypeScript (Phase 7.3)
- DB encryption: option available (Phase 10)
- E2E tests (Phase 9)
